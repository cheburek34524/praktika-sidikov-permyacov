window.LessonModal = function LessonModal({ lesson, subjects, teachers, groups, onSave, onDelete, onClose }) {
    ...
    <div className="modal-bg" onMouseDown={onClose}>
      <div className="modal" onMouseDown={e => e.stopPropagation()}>
        ...
        <div className="row">
          ...
          <select value={form.subjectId} onChange={e => {
            ...
            {subjects.map(s => {
              ...
              return <option key={s.id} value={s.id}>{s.name} ({g?.name || "?"})</option>;
            })}
          </select>
        </div>
        <div className="row">
        ...
        <div className="row">
          ...
          <input value={form.room} placeholder="например, 305" onChange={e => set("room", e.target.value)} />
        </div>
        <div className="row">
        ...
        <div className="actions">
          {lesson.id && <button className="danger" onClick={() => onDelete(lesson.id)}>Удалить</button>}
          <button onClick={onClose}>Отмена</button>
          <button className="primary" onClick={() => {
            if (!form.subjectId || !form.teacherId) { alert("Выберите дисциплину и преподавателя"); return; }
            onSave(form);
          }}>Сохранить</button>
        </div>
      </div>
